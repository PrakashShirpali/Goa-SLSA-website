...
@section('content')
    <form class="form" action="{{ route('GslsaNorthMembers.store') }}" method="POST">
        @csrf
            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($GslsaNorthMembers as $GslsaNorthMember)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $GslsaNorthMember->role }}</td>
                            <td>{{ $GslsaNorthMember->title }}</td>
                            <td>{{ $GslsaNorthMember->name }}</td>
                            <td>{{ $GslsaNorthMember->post }}</td>
                            <td>{{ $GslsaNorthMember->image_path }}</td>
                            <td>{{ $GslsaNorthMember->role_order }}</td>
                            <td>{{ $GslsaNorthMember->priority }}</td>
                            <td class="d-flex gap-1">
                                <form action="{{ route('GslsaNorthMembers.edit', compact('GslsaNorthMember')) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-primary bi-pencil-fill"></button>
                                </form>

                                <form action="{{ route('GslsaNorthMembers.destroy', compact('GslsaNorthMember')) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
