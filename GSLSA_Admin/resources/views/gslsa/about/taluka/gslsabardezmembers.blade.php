...
@section('content')
    <form class="form" action="{{ route('GslsaBardezMembers.store') }}" method="POST">
        @csrf
            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($GslsaBardezMembers as $GslsaBardezMember)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $GslsaBardezMember->role }}</td>
                            <td>{{ $GslsaBardezMember->title }}</td>
                            <td>{{ $GslsaBardezMember->name }}</td>
                            <td>{{ $GslsaBardezMember->post }}</td>
                            <td>{{ $GslsaBardezMember->image_path }}</td>
                            <td>{{ $GslsaBardezMember->role_order }}</td>
                            <td>{{ $GslsaBardezMember->priority }}</td>
                            <td class="d-flex gap-1">
                                <form action="{{ route('GslsaBardezMembers.edit', compact('GslsaBardezMember')) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-primary bi-pencil-fill"></button>
                                </form>

                                <form action="{{ route('GslsaBardezMembers.destroy', compact('GslsaBardezMember')) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
