...
@section('content')
    <form class="form" action="{{ route('GslsaMormugaoMembers.store') }}" method="POST">
        @csrf
            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($GslsaMormugaoMembers as $GslsaMormugaoMember)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $GslsaMormugaoMember->role }}</td>
                            <td>{{ $GslsaMormugaoMember->title }}</td>
                            <td>{{ $GslsaMormugaoMember->name }}</td>
                            <td>{{ $GslsaMormugaoMember->post }}</td>
                            <td>{{ $GslsaMormugaoMember->image_path }}</td>
                            <td>{{ $GslsaMormugaoMember->role_order }}</td>
                            <td>{{ $GslsaMormugaoMember->priority }}</td>
                            <td class="d-flex gap-1">
                                <form action="{{ route('GslsaMormugaoMembers.edit', compact('GslsaMormugaoMember')) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-primary bi-pencil-fill"></button>
                                </form>

                                <form action="{{ route('GslsaMormugaoMembers.destroy', compact('GslsaMormugaoMember')) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
