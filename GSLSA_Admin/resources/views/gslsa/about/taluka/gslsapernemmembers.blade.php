...
@section('content')
    <form class="form" action="{{ route('GslsaPernemMembers.store') }}" method="POST">
        @csrf
            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($GslsaPernemMembers as $GslsaPernemMember)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $GslsaPernemMember->role }}</td>
                            <td>{{ $GslsaPernemMember->title }}</td>
                            <td>{{ $GslsaPernemMember->name }}</td>
                            <td>{{ $GslsaPernemMember->post }}</td>
                            <td>{{ $GslsaPernemMember->image_path }}</td>
                            <td>{{ $GslsaPernemMember->role_order }}</td>
                            <td>{{ $GslsaPernemMember->priority }}</td>
                            <td class="d-flex gap-1">
                                <form action="{{ route('GslsaPernemMembers.edit', compact('GslsaPernemMember')) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-primary bi-pencil-fill"></button>
                                </form>

                                <form action="{{ route('GslsaPernemMembers.destroy', compact('GslsaPernemMember')) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
