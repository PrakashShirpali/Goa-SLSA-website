...
@section('content')
    <form class="form" action="{{ route('GslsaBicholimMembers.store') }}" method="POST">
        @csrf
            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($GslsaBicholimMembers as $GslsaBicholimMember)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $GslsaBicholimMember->role }}</td>
                            <td>{{ $GslsaBicholimMember->title }}</td>
                            <td>{{ $GslsaBicholimMember->name }}</td>
                            <td>{{ $GslsaBicholimMember->post }}</td>
                            <td>{{ $GslsaBicholimMember->image_path }}</td>
                            <td>{{ $GslsaBicholimMember->role_order }}</td>
                            <td>{{ $GslsaBicholimMember->priority }}</td>
                            <td class="d-flex gap-1">
                                <form action="{{ route('GslsaBicholimMembers.edit', compact('GslsaBicholimMember')) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-primary bi-pencil-fill"></button>
                                </form>

                                <form action="{{ route('GslsaBicholimMembers.destroy', compact('GslsaBicholimMember')) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
