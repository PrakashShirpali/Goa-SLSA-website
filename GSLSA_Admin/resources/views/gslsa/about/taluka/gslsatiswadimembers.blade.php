...
@section('content')
    <form class="form" action="{{ route('GslsaTiswadiMembers.store') }}" method="POST">
        @csrf
            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($GslsaTiswadiMembers as $GslsaTiswadiMember)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $GslsaTiswadiMember->role }}</td>
                            <td>{{ $GslsaTiswadiMember->title }}</td>
                            <td>{{ $GslsaTiswadiMember->name }}</td>
                            <td>{{ $GslsaTiswadiMember->post }}</td>
                            <td>{{ $GslsaTiswadiMember->image_path }}</td>
                            <td>{{ $GslsaTiswadiMember->role_order }}</td>
                            <td>{{ $GslsaTiswadiMember->priority }}</td>
                            <td class="d-flex gap-1">
                                <form action="{{ route('GslsaTiswadiMembers.edit', compact('GslsaTiswadiMember')) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-primary bi-pencil-fill"></button>
                                </form>

                                <form action="{{ route('GslsaTiswadiMembers.destroy', compact('GslsaTiswadiMember')) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
