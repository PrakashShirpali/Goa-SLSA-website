...
@section('content')
    <form class="form" action="{{ route('GslsaCanaconaMembers.store') }}" method="POST">
        @csrf
            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($GslsaCanaconaMembers as $GslsaCanaconaMember)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $GslsaCanaconaMember->role }}</td>
                            <td>{{ $GslsaCanaconaMember->title }}</td>
                            <td>{{ $GslsaCanaconaMember->name }}</td>
                            <td>{{ $GslsaCanaconaMember->post }}</td>
                            <td>{{ $GslsaCanaconaMember->image_path }}</td>
                            <td>{{ $GslsaCanaconaMember->role_order }}</td>
                            <td>{{ $GslsaCanaconaMember->priority }}</td>
                            <td class="d-flex gap-1">
                                <form action="{{ route('GslsaCanaconaMembers.edit', compact('GslsaCanaconaMember')) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-primary bi-pencil-fill"></button>
                                </form>

                                <form action="{{ route('GslsaCanaconaMembers.destroy', compact('GslsaCanaconaMember')) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
